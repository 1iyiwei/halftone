...
int FrameBuffer::WriteFIG(const vector<L2F> & samples, const vector<P3> & palette, const vector<float> & region, const float radius, const int boundary, const int color_scheme, const char *file_name)
{
    ...
    return WriteFIG(samples, tiles, palette, region, radius, boundary, color_scheme, file_name);
}

int FrameBuffer::WriteFIG(const vector<Tile> & tiles, const vector<P3> & palette, const vector<float> & region, const int boundary, const int color_scheme, const char *file_name)
{
    vector<L2F> samples;

    return WriteFIG(samples, tiles, palette, region, 0, boundary, color_scheme, file_name);
}

int FrameBuffer::WriteFIG(const vector<L2F> & samples, const vector<Tile> & tiles, const vector<P3> & palette, const vector<float> & region, const float radius, const int boundary, const int color_scheme, const char *file_name)
{
    ofstream output(file_name);
    ...
    else
    {
        ...
        { 
            ...
            {
                output << floor(tiles[i].vertices[j%tiles[i].vertices.size()].x*units+0.5) << " " << floor(tiles[i].vertices[j%tiles[i].vertices.size()].y*units+0.5) << " ";
            }
